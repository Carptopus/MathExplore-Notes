...
\label{thm:twofibre}
Suppose \(f:F\to F^*\) is exactly two-to-one with image \(Y\), the restriction
\(x\mapsto xf(x)\) from \(F^*\) to \(F^*\) is a permutation, and every fibre
\(\{x,x'\}\) satisfies
\begin{equation}
 \Tr((x+x')f(x))=1.
 \label{eq:fibretrace}
...
is a cyclic \((v,k,\lambda)=(q-1,q/2,q/4)\) difference set.  Apply the
multiplier theorem to its complement \(D=F^*\setminus Y\), whose parameters
are
\[
 (v,k',\lambda',n)=(q-1,q/2-1,q/4-1,q/4).
\]
Here \(n\) is a power of the prime \(2\), \(n>\lambda'\), and
...
of \(D\), and hence also of \(Y\).  Thus \(Y^2=\alpha Y\) for some
\(\alpha\in F^*\).
...
The deletion--exchange construction of J\"ager et al.
\cite[Construction~5.2 and Theorem~5.3]{jager-youden} is an especially close
prior framework.  Applied to a Youden rectangle with
\((n,k,\lambda)=(4t-1,2t-1,t-1)\), it gives exactly the dimensions,
replication, and column concurrence in \eqref{eq:tparams}.  What it guarantees
automatically is the binary equireplicate mono-array layer; constant
row--column concurrence and properness require an additional compatibility.
The contribution here is not the deletion--exchange transformation, but a
compatible proper ordering for every power-of-two \(t\), supplied by
Theorem~\ref{thm:twofibre} and the maps constructed in the preceding sections.
...
\cite{flocks-ovals,kolsch,kolsch-correction,abdukhalikov}.
...
Bagchi--Bagchi \cite{bagchi-ovals} construct triple arrays from ovals in
projective planes of odd order, with dimensions \((n+1)\times n^2\) and
\(n(n+1)\) symbols.  That odd-order triple-array family has different
parameters and constant row concurrence, so it does not supply the proper
ordering or the power-of-two family proved here.
...
\url{https://dmtcs.episciences.org/11000}.
\bibitem{kolsch}
...
\bibitem{kolsch-correction}
L.~Kölsch and G.~Kyureghyan,
Correction: The classifications of o-monomials and of 2-to-1 binomials are
equivalent,
\emph{Des. Codes Cryptogr.} 93 (2025), 2895--2896,
\url{https://doi.org/10.1007/s10623-025-01645-5}.
The correction concerns the displayed polynomial \(F_7\) in their
Theorem~4.1; the present manuscript uses their Theorem~2.2.
\bibitem{bagchi-ovals}
S.~Bagchi and B.~Bagchi,
Triple arrays from ovals in finite projective planes,
arXiv:2607.20275 (2026),
\url{https://arxiv.org/abs/2607.20275}.
\bibitem{mcsorley}
