"""Exact certificate for the compact-bulk fourth-order relative margin.

For the anti-diagonal saddlepoint expansion, write

    log(coefficient ratio / target weight)
        = M/k + C2/k^2 + C3/k^3 + C4/k^4 + O(k^-5).

This script reconstructs C4 from the Fourier--Gaussian expansion and proves

    C4 > -4 M

uniformly on 1/2 <= mu <= 12.  The proof uses only rational arithmetic and
Bernstein coefficients on the same saddle strip as the second- and
third-order certificates.  It does not bound the remaining O(k^-5) term.
"""
...
def derive_fourth_coefficient() -> tuple[sp.Expr, sp.Symbol, sp.Symbol]:
    ...
    cumulants = {order: sp.symbols(f"kappa_{order}") for order in range(3, 9)}
    ...
    expansion = sp.series(sp.exp(exponent), epsilon, 0, 9).removeO().expand()
    probability = {
        order: gaussian_integral(expansion.coeff(epsilon, 2 * order), u, variance)
        for order in range(1, 5)
    }
    placeholder = sum(
        probability[order] * inverse_k**order for order in range(1, 5)
    )
    logarithm = sp.series(
        sp.log(1 + placeholder), inverse_k, 0, 5
    ).removeO().expand()
    log_terms = {
    ...
    probability_fourth = probability_ratio.coeff(inverse_k, 4)
    ...
    target_fourth = sp.series(
        sp.log(target), inverse_k, 0, 5
    ).removeO().expand().coeff(inverse_k, 4)
    return sp.factor(probability_fourth - target_fourth), mu, variance


def reduce_relative_margin() -> sp.Poly:
    coefficient, mu_symbol, variance_symbol = derive_fourth_coefficient()
    ...
    numerator, denominator = sp.together(
        coefficient.subs(substitutions) + 4 * margin
    ).as_numer_denom()
    base = 4 * f * z - 5 * f + 1
    assert sp.factor(denominator - 48 * base**10) == 0
    print("PASS: exact fourth-order saddlepoint reduction")
...
def verify_positive_strip(polynomial: sp.Poly) -> None:
    ...
    # Substitute f=L_40+t*z^41/(46(1-z)), multiply by the positive
    # (1-z)^f_degree, and certify positivity on the full (z,t) rectangle.
    ...
    print("PASS: rational Bernstein certificate gives C4 > -4 M")


if __name__ == "__main__":
    verify_saddle_interval()
    relative_polynomial = reduce_relative_margin()
    verify_positive_strip(relative_polynomial)
