...
$scriptPath = Join-Path $PSScriptRoot 'verify_corank_one_parallel_support.py'
$expectedPath = Join-Path $PSScriptRoot 'results\verification-output.txt'

$ordinary = @(& $pythonCommand.Source -B $scriptPath)
if ($LASTEXITCODE -ne 0) {
    throw "ordinary verification exited with code $LASTEXITCODE"
}

$optimized = @(& $pythonCommand.Source -B -O $scriptPath)
if ($LASTEXITCODE -ne 0) {
