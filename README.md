...
| [Acyclic component hypotheses for total cut complexes of disconnected graphs](research/total-cut-low-components/README.md)<br>[PDF](research/total-cut-low-components/total-cut-low-components.pdf) · [BibTeX](research/total-cut-low-components/CITATION.bib) · [DOI](https://doi.org/10.5281/zenodo.22141830) | Algebraic topology, graph complexes, simplicial complexes | Proves the wedge-of-spheres formula for every $k\geq d$ when all relevant component total-cut complexes are void or integer-acyclic, and answers Question 30 for this class | v0.3-beta | Internally verified candidate proof; external review pending |
...
| [Exact certificates for the first two cases not covered by Steinberger's cyclotomic minimum-degree theorem](research/cyclotomic-minimum-degree-first-two-cases/README.md)<br>[PDF](research/cyclotomic-minimum-degree-first-two-cases/cyclotomic-minimum-degree-first-two-cases.pdf) · [BibTeX](research/cyclotomic-minimum-degree-first-two-cases/CITATION.bib) · [DOI](https://doi.org/10.5281/zenodo.22124096) | Cyclotomic polynomials, nonnegative coefficients, CRT tensor arrays, exact certificates | Proves the conjectured minimum degree and unique monic minimizer for the first two parameters outside Steinberger's general theorem | v0.1-beta | Internally verified candidate proof; external review pending |
...
For $q=2^m$ with odd $m\ge5$, the manuscript proves that the one-dimensional affine stabilizer
of $\{x^e+x:x\in\mathbb F_q\}$ is trivial for both the $\overline{\mathrm{Segre}}$ exponent
$e=q-6$ and the Glynn I exponent $e=3\cdot2^{(m+1)/2}+4$. Together with Ding--Tang's earlier
cases, this covers every currently known nontranslation o-monomial family. It does not classify
hypothetical unknown o-monomials and does not claim the unrestricted Conjecture 2.

- [Manuscript and verification materials](research/o-monomial-affine-stabilizers/README.md)

### Parallel-support rank and the Merino--Welsh inequality
...
The inequality is strict at parallel-support corank one, and equality in the full class occurs
exactly for direct sums of copies of $U_{1,2}$. Dual and graphic forms are included. Explicit
connected and disconnected counterexamples at support corank two show that the rank threshold is
sharp without additional hypotheses.

- [Manuscript and verification materials](research/parallel-support-rank-merino-welsh/README.md)
...
For the independent $t$-fold blow-up of Mader's 12-vertex graph, the manuscript proves the exact
formula
...
Since its minimum degree is $5t$, the gap between minimum degree and the largest connectivity of a
minor equals $\lceil t/2\rceil$ and is unbounded. This supplies infinitely many counterexamples to
Barát's conjectured universal bound $\kappa^*(G)\ge\delta(G)-1$. The proof combines an explicit
uniform minor model with a three-factor torso classification and an anticomplete-shores argument.

- [Manuscript and verification materials](research/unbounded-minor-connectivity-gap/README.md)
...
For every order $n\ge8$, the manuscript constructs four pairwise nonisomorphic connected graphs
with at least as many edges as vertices whose connected set polynomials are not unimodal. A common
rooted-block formula proves the fixed coefficient pattern $10,11,10$. A complete scan of all
11,117 connected eight-vertex graph classes shows that eight is the minimum counterexample order
and that the four first family members are exactly all minimum-order counterexamples.

- [Manuscript and verification materials](research/connected-set-polynomial-counterexamples/README.md)
...
For any finite six-permutation template and any probability weights with at
least two positive entries, the manuscript proves an exact weighted
lexicographic recursion. Applied to the published 26-point template, it gives

$$
c_3\ge\frac{1288385}{2599242}=0.495677201276\ldots,
$$

strictly improving the uniform-recursion value \(482/975\) for the same
template. The included exact certificate reconstructs all 2600 triples and the
rational weighted bound.

- [Manuscript and verification materials](research/six-permutation-weighted-shattering/README.md)
...
### Two-layer antichain polynomials
...
has only negative real zeros, proving Ding and Dong's Conjecture 4.3. It is
palindromic if and only if $|m-n|=1$, and in precisely these adjacent cases all
gamma coefficients are strictly positive. This proves Conjecture 4.5 and the
general gamma-positivity conjecture for the complete subfamily
$k=2$, $P=[m]\times[n]$.

A canonical two-gap tail-switching bijection first establishes the previously
unproved $2\times2$ determinant for the associated Zhang--Zhang polynomial. A
Möbius transformation then factors it into controlled combinations of adjacent
Jacobi polynomials, whose zeros are located by interlacing and endpoint signs.

- [Manuscript and exact verification](research/antichain-box-real-rootedness/README.md)

### Complete palindromicity classification for three-dimensional boxes

For positive integers $a\leq b\leq c$, the antichain generating polynomial of
$[a]\times[b]\times[c]$ is palindromic exactly for

$$
(a,b,c)=(1,r,r)\quad\text{or}\quad(2,r,r+1).
$$

The new higher-layer theorem excludes every box with $a,b,c\geq3$ by comparing
the first and next-to-leading coefficients through nonnegative last-passage
matrices, path complementation and minimum separators. Together with the
one- and two-layer cases, this closes the rectangular minuscule branch of
Ding--Dong Conjecture B.

- [Manuscript and exact verification](research/antichain-box-palindromicity-classification/README.md)
